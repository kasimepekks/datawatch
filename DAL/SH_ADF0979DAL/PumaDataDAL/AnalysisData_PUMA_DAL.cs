...
using Tools.ListOperation.PUMAMileageOperation;
...
{
  public  class AnalysisData_PUMA_DAL : BaseDAL<Pumapermileage>, IAnalysisData_PUMA_IDAL
    {
       
        public AnalysisData_PUMA_DAL(datawatchContext DB) : base(DB)
        {
        ...
        {
            var time = Convert.ToDateTime(file.Name.Split('-')[0].Replace("_", "-"));//如“2021-07-03”
            if (vehicleIDPara.importpuma == 1)
            {
                _DB.vehicleid = vehicleid;
                var sqllist = _DB.Pumapermileages.Where(a => a.filedate >= time  && a.vehicle == vehicleid).Select(a => a.vehicle).AsNoTracking().FirstOrDefault();
                if (string.IsNullOrEmpty(sqllist))
                {
                    if (importstruct.Iscontinue)
                    {
                        ...
                        {
                            var list = PumaMileage.addPumaMileage(importstruct, vehicleid);
                            if (list.Count > 0)
                            {
                    ...
                    {
                        return ($"此文件{file.Name}进行里程模式统计值计算时出现了问题，数据状态为{importstruct.status}！{DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss")}");
                    }
                }
                else
                {
                    return ($"此文件{file.Name}进行里程模式统计值计算时已导入过，须删除后再重新导入！{DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss")}");
                }
            }
            else
            {
                return ($"此统计值分析模块未设置！{DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss")}");
            }
        }
